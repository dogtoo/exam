...
@EnableWebMvc
@Controller
public class QsCrtMastAction {

	private static final String c_progId = "QsCrtMast";
	
	@RequestMapping(value = "/QsCrtMast", method = RequestMethod.GET)
...
//		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/QsCrtMast_qryMemberList", method = RequestMethod.POST)
...
//		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/QsCrtMast_qryQsList", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> qryQsList(@RequestParam Map<String, String> req) {
		...
		try {	
			...
			StringBuffer sqlQryQs = new StringBuffer(
				...
				+ "      , qstmp_id, file_max_size, stat_flag, edit_remark\n"
			...
			StringBuffer sqlCond = new StringBuffer(
				  "   FROM qsmstr a\n"
				+ "  WHERE stat_flag = '" + QsSvc.c_qsMastFlagNew + "'\n");
			...
			// 狀態
			String qryStatFlag = req.get("statFlag");
			if (!qryStatFlag.isEmpty()) {
				sqlCond.append(" AND stat_flag = ?\n");
				params.add(qryStatFlag);
			}
			...
			if (!qryUserId.isEmpty()) {
				sqlCond.append(" AND EXISTS(SELECT user_id FROM qsuser c WHERE a.qs_id = c.qs_id AND c.user_id = ? AND c.end_date > ?)\n");
			...
			Map<String, String> statMap = CodeSvc.buildStringMapByKind(dbu, CodeSvc.c_kindQsMastFlag);
			...
			while (rsQs.next()) {
				...
				qs.put("statFlag", statMap.get(rsQs.getString("stat_flag")));
...
//		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/QsCrtMast_qryQs", method = RequestMethod.POST)
	 ...
	 * @return
	 */
	@RequestMapping(value = "/QsCrtMast_addQs", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> addQs(@RequestParam Map<String, String> req) {
		...
		try {
			...
			for (int i = 0; ; i++) {
				...
				if (!ParamSvc.doesParamExist(dbu, ParamSvc.c_clsQsFileClass, value))
	...
	@RequestMapping(value = "/QsCrtMast_modQs", method = RequestMethod.POST)
...
//		}
	...
	@RequestMapping(value = "/QsCrtMast_delQs", method = RequestMethod.POST)
...
//		}
	...
	@RequestMapping(value = "/QsCrtMast_addMemberCheck", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> addMemberCheck(@RequestParam Map<String, String> req) {
		...
		try {
			...
			if (dbu.selectIntList(sqlCntUser, req.get("qsId"), userId, userRole, today, today) > 0)
	 ...
	 * @return
	 */
	@RequestMapping(value = "/QsCrtMast_editMode", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> editMode(@RequestParam Map<String, String> req, HttpServletRequest reqData) {
		...
		try {
			...
			QsSvc.multiCheckQs(dbu, qsId, QsSvc.c_qsRoleAdmin, QsSvc.c_qsMastFlagNew);
